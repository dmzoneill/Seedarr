...
- [x] **Workpackage 12: Bandwidth Scheduling, Alternative Speed Windows & Real-Time Charting Parity**
  - [x] **12.1 Interactive 24x7 Bandwidth Speed Scheduler**: Standardized `SpeedSchedule.tsx` across both solutions featuring drag-to-select range painting, touch support, single-click rule toggles, active rate limit cards (active schedule, upload/download limits, paused indicators), delete confirmations with `useConfirm`, error and success feedback with `useToast`, keyboard Escape modal dismissal with `useEscapeKey`, day presets ("All 7 Days", "Mon - Fri", "Sat - Sun"), and full 20-language i18n synchronization.
  - [x] **12.2 Alternative Speed & Turtle Mode Quick Settings**: Aligned `BandwidthCard.tsx` quick controls with localized labels, turtle mode toggle, non-linear rate limits, preset chips, and optimistic configuration persistence across both repositories.
  - [x] **12.3 High-Fidelity Charting & Telemetry Synchronization**: Synchronized real-time throughput metrics, bandwidth scheduling data transfer hooks, and time matrix rendering.
