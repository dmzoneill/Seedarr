...
[TestFixture]
public class LogNormalDistributorTest
{
    private LogNormalDistributor _distributor;

    [SetUp]
    public void Setup()
    {
        _distributor = new LogNormalDistributor();
    }

    [Test]
    public void Name_should_return_lognormal()
    {
        Assert.That(_distributor.Name, Is.EqualTo("LogNormal"));
    }
    ...
    [Test]
    public void Distribute_should_return_single_element_for_one_torrent()
    {
    ...
    [Test]
    public void Constructor_should_clamp_sigma_below_minimum()
    {
        var distributor = new LogNormalDistributor(0.01);
    ...
    [Test]
    public void Constructor_should_clamp_sigma_above_maximum()
    {
        var distributor = new LogNormalDistributor(10.0);
        var speeds = distributor.Distribute(1_000_000L, 5);
    ...
    [Test]
    public void Higher_sigma_should_produce_more_variance()
    {
        var lowSigma = new LogNormalDistributor(0.1);
        var highSigma = new LogNormalDistributor(2.0);

        var lowSpeeds = lowSigma.Distribute(1_000_000L, 10);
        var highSpeeds = highSigma.Distribute(1_000_000L, 10);
