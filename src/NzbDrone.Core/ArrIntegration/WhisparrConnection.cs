...
public class WhisparrConnection : IArrConnection
{
    private readonly HttpClient _explicitClient;
    private readonly ResiliencePipeline _policy;
    private readonly Logger _logger;
    private string _url = "http://localhost:6969";

    public WhisparrConnection(HttpClient client = null, ResiliencePipeline policy = null)
    {
    ...
    public string Name => "Whisparr";
    public string ArrType => "Whisparr";
    ...
    public async Task<List<ArrDownloadRecord>> GetDownloadHistoryAsync(int pageSize, CancellationToken cancellationToken = default)
    {
            ...
            var result = await _policy.ExecuteAsync<string>(
                async ct =>
                {
                    using var request = new HttpRequestMessage(
                        HttpMethod.Get,
                        $"{Url.TrimEnd('/')}/api/v3/history?pageSize={effectivePageSize}&sortKey=date&sortDirection=descending");
                    request.Headers.Add("X-Api-Key", ApiKey ?? "");

                    using var response = await Client.SendAsync(request, ct).ConfigureAwait(false);
                    if (!response.IsSuccessStatusCode)
                    {
                        _logger.Warn("Whisparr API returned {0}", response.StatusCode);
                        return null;
                    }
        ...
        {
            _logger.Error(ex, "Failed to fetch Whisparr history");
            return new List<ArrDownloadRecord>();
        }
    }
    ...
    private List<ArrDownloadRecord> ParseDownloadHistory(string json)
    {
        ...
        _logger.Debug("Fetched {0} download records from Whisparr", records.Count);
        return records;
    }
        ...
        if (root.TryGetProperty("actors", out var actorsArray))
        {
            foreach (var actorElem in actorsArray.EnumerateArray())
            {
                ...
                if (actorElem.TryGetProperty("images", out var actorImgs))
                {
                    ...
                    {
                        headshotUrl = ai.TryGetProperty("remoteUrl", out var aru)
                            ? aru.GetString()
                            : (ai.TryGetProperty("url", out var alu) ? alu.GetString() : null);
    ...
    private static void ExtractAdultFields(JsonElement root, MediaMetadata metadata)
    {
        if (root.TryGetProperty("studio", out var std))
        {
            if (std.ValueKind == JsonValueKind.String)
            {
                metadata.Studio = std.GetString();
            }
            else if (std.ValueKind == JsonValueKind.Object)
            {
                if (std.TryGetProperty("title", out var stTitle))
                {
                    metadata.Studio = stTitle.GetString();
                }
                else if (std.TryGetProperty("name", out var stName))
                {
                    metadata.Studio = stName.GetString();
                }
            }
        }
        else if (root.TryGetProperty("studioTitle", out var stt))
        {
            metadata.Studio = stt.GetString();
        }

        metadata.StudioOrNetwork = metadata.Studio;

        if (root.TryGetProperty("siteName", out var sn))
        {
            metadata.SiteName = sn.GetString();
        }
        else if (root.TryGetProperty("site", out var siteElem))
        {
            if (siteElem.ValueKind == JsonValueKind.String)
            {
                metadata.SiteName = siteElem.GetString();
            }
            else if (siteElem.ValueKind == JsonValueKind.Object && siteElem.TryGetProperty("name", out var sName))
            {
                metadata.SiteName = sName.GetString();
            }
        }

        if (root.TryGetProperty("performers", out var perfElem))
        {
            if (perfElem.ValueKind == JsonValueKind.String)
            {
                metadata.Performers = perfElem.GetString();
            }
            else if (perfElem.ValueKind == JsonValueKind.Array)
            {
                var performersList = new List<string>();
                foreach (var p in perfElem.EnumerateArray())
                {
                    ...
                    else if (p.ValueKind == JsonValueKind.Object)
                    {
                        ...
                        {
                            var nameStr = pName.GetString();
                            if (!string.IsNullOrWhiteSpace(nameStr))
                            {
                                performersList.Add(nameStr.Trim());
                            }
                        }
                    }
                }
                ...
                {
                    metadata.Performers = string.Join(", ", performersList);
                }
            }
        }

        if (root.TryGetProperty("sceneCode", out var scElem))
        {
            metadata.SceneCode = scElem.GetString();
        }
        else if (root.TryGetProperty("foreignId", out var fiElem))
        {
            metadata.SceneCode = fiElem.GetString();
        }

        if (root.TryGetProperty("releaseDate", out var rdElem))
        {
            if (rdElem.TryGetDateTime(out var rdVal))
            {
                metadata.ReleaseDate = rdVal;
            }
            else if (DateTime.TryParse(rdElem.GetString(), out var parsedRd))
            {
                metadata.ReleaseDate = parsedRd;
            }
        }
        else if (root.TryGetProperty("inCinemas", out var icElem) && icElem.TryGetDateTime(out var icVal))
        {
            metadata.ReleaseDate = icVal;
        }
    }
    ...
    public async Task<MediaMetadata> LookupMediaAsync(string title, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.Warn(ex, "Failed to lookup movie by term '{0}' on Whisparr", title);
            return null;
        }
    }
    ...
    public ArrTestResult TestConnectionDetailed()
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                return ArrTestResult.Ok($"Successfully connected to Whisparr at {normalizedUrl}");
            }
            ...
            return ArrTestResult.Fail($"Whisparr returned HTTP {(int)response.StatusCode} {response.ReasonPhrase}");
        }
        catch (HttpRequestException ex)
        {
            _logger.Error(ex, "Whisparr connection test failed: {0}", ex.Message);
            return ArrTestResult.Fail($"Network error: {ex.Message}");
        }
        catch (TaskCanceledException)
        {
            _logger.Error("Whisparr connection test timed out");
            return ArrTestResult.Fail($"Connection timed out connecting to {normalizedUrl} (exceeded 10s)");
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Whisparr connection test failed");
