...
function TorrentContextMenu({
  ...
  onStart, onStop, onUpdate, onAnnounce, onRecheck, onDelete, onMoveQueue,
    ...
    <div className="context-menu" style={{ left: x, top: y }} onClick={(e) => e.stopPropagation()}>
      {ct ? (
        <>
          {/* Pause / Resume */}
          {ct.active ? (
            <button className="context-menu-item" onClick={() => { onStop(ct.id); onClose(); }}>Pause</button>
          ) : (
            <button className="context-menu-item" onClick={() => { onStart(ct.id); onClose(); }}>Resume</button>
          )}
          <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, forceStart: !ct.forceStart }); onClose(); }}>
            {ct.forceStart ? '✓ ' : ''}Force Start
          </button>
          <button className="context-menu-item" onClick={() => { onAnnounce(ct.id); onClose(); }}>Update Tracker</button>
          <button className="context-menu-item" onClick={() => { onRecheck(ct.id); onClose(); }}>Force Recheck</button>
          {ct.progress < 1.0 && (
            <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, progress: 1.0 }); onClose(); }}>Force Complete</button>
          )}
          ...
          <div className="context-menu-item context-menu-submenu-trigger" onMouseEnter={() => setOpenSubmenu('priority')} onMouseLeave={() => setOpenSubmenu(null)}>
            ...
            {openSubmenu === 'priority' && (
              <div className="context-menu context-menu-submenu">
                <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, priority: 2 }); onClose(); }}>{ct.priority === 2 ? '✓ ' : ''}High</button>
                <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, priority: 1 }); onClose(); }}>{ct.priority === 1 ? '✓ ' : ''}Normal</button>
                <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, priority: 0 }); onClose(); }}>{ct.priority === 0 ? '✓ ' : ''}Low</button>
              </div>
            )}
          </div>
          ...
          <div className="context-menu-item context-menu-submenu-trigger" onMouseEnter={() => setOpenSubmenu('queue')} onMouseLeave={() => setOpenSubmenu(null)}>
            ...
            {openSubmenu === 'queue' && (
              <div className="context-menu context-menu-submenu">
                <button className="context-menu-item" onClick={() => { onMoveQueue({ id: ct.id, position: 'top' }); onClose(); }}>Top</button>
                <button className="context-menu-item" onClick={() => { onMoveQueue({ id: ct.id, position: 'up' }); onClose(); }}>Up</button>
                <button className="context-menu-item" onClick={() => { onMoveQueue({ id: ct.id, position: 'down' }); onClose(); }}>Down</button>
                <button className="context-menu-item" onClick={() => { onMoveQueue({ id: ct.id, position: 'bottom' }); onClose(); }}>Bottom</button>
              </div>
            )}
          </div>
          ...
          <button className="context-menu-item" onClick={() => { const n = window.prompt('Rename torrent:', ct.name); if (n !== null && n.trim()) onUpdate({ ...ct, name: n.trim() }); onClose(); }}>Rename...</button>
          <button className="context-menu-item" onClick={() => { const l = window.prompt('Set label:', ct.label ?? ''); if (l !== null) onUpdate({ ...ct, label: l || null }); onClose(); }}>
            Set Label...{ct.label ? ` (${ct.label})` : ''}
          </button>

          <div className="context-menu-separator" />

          <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, superSeeding: !ct.superSeeding }); onClose(); }}>
            {ct.superSeeding ? 'Disable' : 'Enable'} Super Seeding
          </button>
          <button className="context-menu-item" onClick={() => { onUpdate({ ...ct, sequentialDownload: !ct.sequentialDownload }); onClose(); }}>
          ...
          <div className="context-menu-item context-menu-submenu-trigger" onMouseEnter={() => setOpenSubmenu('remove')} onMouseLeave={() => setOpenSubmenu(null)}>
            ...
            {openSubmenu === 'remove' && (
              <div className="context-menu context-menu-submenu">
                <button className="context-menu-item context-menu-item-danger" onClick={() => { if (confirm(`Remove "${ct.name}"?`)) onDelete({ id: ct.id }); onClose(); }}>Remove Torrent</button>
                <button className="context-menu-item context-menu-item-danger" onClick={() => { if (confirm(`Remove "${ct.name}" and all data?`)) onDelete({ id: ct.id, deleteFiles: true }); onClose(); }}>Remove Torrent and Data</button>
              </div>
            )}
          </div>
