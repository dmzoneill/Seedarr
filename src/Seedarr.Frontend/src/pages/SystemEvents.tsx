...
function SortArrow({ direction }: { direction: 'asc' | 'desc' }) {
  return (
    <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" style={{ marginLeft: '4px', opacity: 0.6 }}>
      {direction === 'asc' ? (
    ...
    case 'Warn':
      return (
        <span className="event-level-icon event-level-warn" title="Warning">
          <svg width="16" height="16" viewBox="0 0 16 16">
            <polygon points="8,1 15,15 1,15" fill="currentColor" opacity="0.2" />
            <polygon points="8,1 15,15 1,15" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
            <text x="8" y="13" textAnchor="middle" fill="currentColor" fontSize="10" fontWeight="700" fontFamily="sans-serif">!</text>
          </svg>
        </span>
      );
    case 'Error':
        ...
        <table className="torrent-table">
          <thead>
            <tr>
              ...
              <th className="torrent-table-th events-time-col" onClick={toggleSort} style={{ width: '100px' }}>
                Time
                <SortArrow direction={sortDirection} />
              </th>
              <th className="torrent-table-th" style={{ width: '180px' }}>Component</th>
