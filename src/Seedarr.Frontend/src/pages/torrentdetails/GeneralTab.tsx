  ...
  return (
    <div className="detail-grid">
      <div className="card">
        ...
        <StatusRow label="Pieces">{torrent.pieceCount} x {formatBytes(torrent.pieceLength)}</StatusRow>
        <StatusRow label="Private">{torrent.isPrivate ? 'Yes' : 'No'}</StatusRow>
        <StatusRow label="Created">{formatDate(torrent.creationDate)}</StatusRow>
        {torrent.createdBy && <StatusRow label="Created By">{torrent.createdBy}</StatusRow>}
        {torrent.comment && <StatusRow label="Comment">{torrent.comment}</StatusRow>}
        {torrent.sourcePath && <StatusRow label="Source Path" mono>{torrent.sourcePath}</StatusRow>}
      </div>
      ...
      <div className="card">
        ...
        <StatusRow label="Last Active">{formatDate(torrent.lastActive)}</StatusRow>
      </div>
    </div>
  );
}
