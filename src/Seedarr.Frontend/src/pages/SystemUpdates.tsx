...
function SystemUpdates() {
  ...
  const isUpToDate = updates && updates.length > 0 && updates.every((u) => !u.latest || u.installed);
    ...
    <div>
      ...
      {updates && (
        <>
          ...
          {updates.map((update) => (
            <div key={update.version} className="card" style={{ padding: '1rem 1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                ...
                {update.installed && (
                  <span className="badge badge-seeding" style={{ marginLeft: '0.25rem' }}>
                    Currently Installed
                  </span>
                )}
                {update.latest && !update.installed && (
                  <span className="badge badge-queued" style={{ marginLeft: '0.25rem' }}>
                    Latest
                  </span>
                )}
              </div>

              {update.changes && update.changes.new && update.changes.new.length > 0 && (
              ...
              {update.changes && update.changes.fixed && update.changes.fixed.length > 0 && (
                <div>
                  <div style={{
                    fontSize: '0.75rem',
                    fontWeight: 600,
