...
export function useTorrentIndexState() {
  ...
  const stateCounts = useMemo(() => {
    const all = torrents ?? [];
    const counts: Record<string, number> = { All: all.length, Seeding: 0, Stopped: 0, Queued: 0, Error: 0 };
    for (const t of all) { if (t.status in counts) counts[t.status]++; }
    return counts;
  }, [torrents]);
  ...
  const { totalUploadSpeed, totalDownloadSpeed } = useMemo(() => {
    let ul = 0; let dl = 0;
    for (const t of torrents ?? []) { ul += t.uploadSpeed ?? 0; dl += t.downloadSpeed ?? 0; }
