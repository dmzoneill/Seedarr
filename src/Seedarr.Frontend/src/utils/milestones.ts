 ...
 */
export function getTorrentBadges(torrent: Torrent): { label: string; icon: string; title: string; color: string }[] {
  const badges: { label: string; icon: string; title: string; color: string }[] = [];
  ...
  } else if (torrent.seedingTime >= 2592000) { // 30+ days
    badges.push({
      label: "30d+",
      icon: "💎",
...
export function calculateAchievements(
  ...
  const swarmGuardians = tList.filter((t) => (t.seeders ?? 0) <= 2 && t.status === "Seeding");
...
export function calculateTrackerBuffers(torrents: Torrent[] | undefined): TrackerBufferSummary[] {
  ...
  return Object.entries(trackerMap).map(([tracker, data]) => {
    const ratio = data.downloaded > 0 ? data.uploaded / data.downloaded : data.uploaded > 0 ? 10.0 : 0;
    ...
    data.torrents.forEach((t) => {
      ...
      const pts = Math.pow(Math.max(0.1, sizeGb), 0.55) * (1 + seedDays / 10) * (1 / Math.pow(seeders, 0.5));
